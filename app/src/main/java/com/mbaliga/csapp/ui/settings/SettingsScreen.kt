    ...
    Scaffold(topBar = { TopAppBar(title = { Text("Settings") }) }) { padding ->
        Column(modifier = Modifier.padding(padding).padding(16.dp).verticalScroll(rememberScrollState())) {
            ...
            Text(
                ...
                // Debug-only affordance: long-press previews the shared crash-recovery screen
                // with sample content (no real crash, no disk read/write) so its look can be
                // reviewed without crashing the app. Never reachable from a release build —
                // isDebugBuild gates both the modifier and the resulting intent launch.
                modifier = Modifier
                    .padding(top = 4.dp)
                    .combinedClickable(
                        ...
                        onLongClick = {
                            context.startActivity(
                                CrashRecovery.previewIntent(context, appLabel = "csapp", style = CsAppCrashRecoveryStyle),
                            )
                        },
                    ),
            )
